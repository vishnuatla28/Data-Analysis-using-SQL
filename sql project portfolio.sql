...
select * from [sql project]..data1 where state in ('Jharkhand' ,'Bihar')
...
select state,round(avg(sex_ratio),0) avg_sex_ratio from [sql project]..data1 group by state order by avg_sex_ratio desc;
...
select state,avg(growth)*100 avg_growth from [sql project]..data1 group by state order by avg_growth desc;
...
(select a.district,a.state,a.sex_ratio/1000 sex_ratio,b.population from [sql project]..data1 a inner join [sql project]..data2 b on a.district=b.district ) c) d
group by d.state;
...
(select a.district,a.state,a.growth growth,b.population from [sql project]..data1 a inner join [sql project]..data2 b on a.district=b.district) d) e
group by e.state)m
